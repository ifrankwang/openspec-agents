...
- **豁免机制**：developer 通过 `exempt_issue_ids` 申请豁免，issue 进入 review（待裁定）态 → 报 issue 的 review step 通过 `exempt_adjudications` 裁定（dismissed→cancelled、rejected→回修）
- **校验守卫**：多维度校验确保流程完整性
