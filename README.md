...
4. **收尾合并**：任务组完成只做门禁校验并标记变更范围，不合并、不销毁 worktree；最后一个任务组收口时把 change 分支一次性合并回基准分支，随后销毁 worktree 并删除 change 分支。合并前若基准分支已推进，先做基线漂移检查，发现漂移或冲突则把该任务组回退到收尾验证环节（合并基准分支最新代码并回归复验）后再收口——验收仅在漂移时动态发生，正常路径零额外环节。
