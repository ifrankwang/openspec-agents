...
describe("step 操作层指引补全", () => {
  test("common：文档读取路径 / AGENTS.md / opx_orch_* 禁令 / 变更范围命令", () => {
    ...
    expect(out).toContain("以视图「变更范围」命令产出的文件清单作为本次实施/修改范围；审查/报告范围按各 step 指令执行")
  ...
  test("verify_tool：工具检查全量 / 跨维归因 / 待裁定 / 工具边界 / passed 门禁", () => {
    ...
    expect(out).toContain("有实质变更时，工具检查及 issue 报告覆盖全量代码；非本轮变更文件的工具违规同样必须映射为 issue（命中项目级豁免清单的按 Info，不重新豁免；其余非本轮引入问题按原严重级别上报），不得以不在变更清单或未越界修改为由丢弃")
