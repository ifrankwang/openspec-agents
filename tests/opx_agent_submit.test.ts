...
describe("opx_agent_submit 通用 step 提交", () => {
  ...
  test("5. new_children → 写回 item.children 新增 todo child", async () => {
    ...
    try {
      ...
      await agent_submit.execute(
        {
          change_id: CID, step_id: "implement", verdict: "passed", completed_task_ids: ["1", "2", "3"],
          new_children: [{ id: "7", title: "新发现问题", description: "dev 提交时新增 issue", severity: "Low" }],
        },
        makeCtx("openspec-developer", wt)
      )
      ...
      expect(child.phase).toBe("todo")
      expect(child.description).toBe("dev 提交时新增 issue")
      // 新 child 未终态 → 该 step 不推进
      expect(taskItemOf(wt).phase).toBe("in_progress")
    } finally {
      try { rmSync(root, { recursive: true, force: true }) } catch {}
    }
  ...
  test("10. done 终态：全部 task verified + children 全终态，complete_task_group 收尾置 completed 并 merge", async () => {
    ...
    try {
      ...
      // task 全 verified
      expect(item.metadata.tasks.every((t: any) => t.status === "verified")).toBe(true)
  ...
  test("13. new_children 透传 source_phase/dimension 到 child.metadata，缺省 tool/style", async () => {
    ...
    try {
      ...
      await agent_submit.execute(
        {
          change_id: CID, step_id: "implement", verdict: "passed", completed_task_ids: ["1", "2", "3"],
          new_children: [
            { id: "7", title: "注入风险", description: "安全问题", severity: "High", source_phase: "quality", dimension: "security" },
      ...
      expect(c7.metadata["source"]).toBe("openspec-developer")
  ...
  test("15. 豁免裁定 dismissed：dev 申请豁免后，匹配 reviewer_for 的 quality reviewer 用 exempt_adjudications 裁定 → child cancelled", async () => {
    ...
    try {
      ...
      // style reviewer 报 issue（metadata.source=openspec-reviewer-style）并 failed，verify_quality 回 implement
      await agent_submit.execute(
        {
      ...
      // quality reviewer（style，匹配 verify_quality.reviewer_for）用 exempt_adjudications 裁定 dismissed
      await agent_submit.execute(
        {
          change_id: CID, step_id: "verify_quality", verdict: "passed",
  ...
  test("16. 越权裁定：非 reviewer_for/agents 白名单的 agent 裁定豁免 → 抛错且 state 不变", async () => {
  ...
  test("24. implement completed_task_ids 迁移 open→submitted，metadata.self_check_results 落盘", async () => {
    ...
    try {
      ...
      expect(r).toContain("- **推进**: 是 → verify_tool")
      const tasks = taskItemOf(wt).metadata.tasks as any[]
      expect(tasks.find((t: any) => t.id === "1").status).toBe("submitted")
  ...
  test("25. implement blocker 参数：verdict=failed + blocker → on_fail 回 analyze，tasks 全置 open", async () => {
    ...
    try {
      ...
      await agent_submit.execute({ change_id: CID, step_id: "implement", verdict: "passed", completed_task_ids: ["1", "2", "3"] }, makeCtx("openspec-developer", wt))
      expect((taskItemOf(wt).metadata.tasks as any[]).every((t: any) => t.status === "submitted")).toBe(true)
      ...
      // resetTasksForBlocker：tasks 全 open，review 验证标记清空
      expect((taskItemOf(wt).metadata.tasks as any[]).every((t: any) => t.status === "open")).toBe(true)
      expect(taskItemOf(wt).metadata["blockers"]).toHaveLength(1)
    } finally {
      try { rmSync(root, { recursive: true, force: true }) } catch {}
    }
  ...
  test("26. verify_task verified_tasks/failed_tasks：submitted task 全覆盖校验 + 状态迁移", async () => {
    ...
    try {
      ...
      expect(taskItemOf(wt).phase).toBe("in_progress")
      const tasks = taskItemOf(wt).metadata.tasks as any[]
      expect(tasks.find((t: any) => t.id === "1").status).toBe("verified")
      expect(tasks.find((t: any) => t.id === "3").status).toBe("rejected")
      expect(tasks.find((t: any) => t.id === "3").rejectReason).toBe("验收未过")
    } finally {
      try { rmSync(root, { recursive: true, force: true }) } catch {}
    }
  })
